...
\begin{figure}[h]
	\centering
	\includegraphics[width=3in]{amp-pic}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=3in]{splitter-pic}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=3.5in]{gsp730}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=3.5in]{tpi-synth}
	\caption{Picture of the Instek GSP730 spectrum analyzer.}
	\label{fig:tpi-synth}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=2in]{rf-cable}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=2in]{tnc-sma}
...
\begin{enumerate}
	...
	\item Turn on the spectrum analyzer and configure it as below.
		\begin{enumerate}
			...
			\item Set the reference level. Press the ``Amplitude'' button, then press ``F1'' button to select the ``Ref. Level'' option. Type in ``20 dB'' to set the reference level to 20\,dBm\footnote{The reference level is set to be comparable to the signal input power.} . The screen of the spectrum analyzer should look like the following: 
				\begin{figure}[h]
					\centering
					\includegraphics[width=3.5in]{sa-reference}
					\caption{Set the reference level.}
	...
	\item Measuring a single-tone RF signal
		\begin{enumerate}
			\item Launch the \textit{SynthMachine} software and choose the appropriate calibration file (serial number) for the TPI synthesizers. In the SynthMachine software, set the ``Center Frequency'' to 2400\.MHz , set the ``Output Power'' to 10\,dBm, and click ``RF On'' to turn on the RF signal output. 
				\begin{figure}[h]
					\centering
					\includegraphics[width=4.5in]{synth-machine}
	...
	\item Measuring two RF signals. 
		\begin{enumerate}
			...
			\item The screen of the spectrum analyzer should look like the following. 
				\begin{figure}[h]
					\centering
...
\begin{enumerate}
	...
	\item Connect the ``+5VDC'' pin of the amplifier to the positive terminal of the lab power supply using a wire; connect the ``GND'' pin of the amplifier to the ground of the power supply.
...
Using the same setup as in Lab.~\ref{sec:bw-gain}, we will measure the 1-dB compression point of the amplifier. 
...
\begin{enumerate}
	\item For the IP3 measurement, we need two input signals with a slight frequency offset. Set up the measurement as illustrated by Fig. 2.40.1.
...
\begin{enumerate}
	...
	\item Make sure that you have 50\,Ohm microstrip lines for the input and output SMA connectors.
	...
	\item Your final circuit will look similar, but may not be identical, to the ADL5611 evaluation board (Fig.~\ref{fig:adl5611-evalz}).
	 
		\begin{figure}[h]
			\centering
			\includegraphics[width=4in]{adl5611-evalz}
