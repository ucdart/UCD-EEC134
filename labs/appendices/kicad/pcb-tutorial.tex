...
In KiCad, the schematic symbol and the footprint of a device are stored in separate files (.lib for schematic symbols and .mod for footprint). In order for the PCB layout tool to put the correct footprints in the layout, we need to assign a relationship between the schematic symbols and the footprints in a circuit. 
\begin{enumerate}
	\item Open CvPcb. By default, CvPcb should have loaded the netlist of the project. If not, you can go to “File->Open” to open the desired netlist.
	
	The CvPcb window consists of two panes. The left one lists all the schematic components used in the circuit. The right one presents a list of available footprints (in KiCad terminology, a footprint is called a module). These footprints are installed with the KiCad program. By default, CvPcb only present footprints that it thinks that are relevant to the components. To see a full list of available components, click the “Display the full footprint list (without filtering)” button . 
	Our job in CvPcb is to associate the schematic components with their corresponding footprint. This is done by first selecting the component in the left pane, and then double clicking on the right footprint in the right pane. You can preview the drawing of the footprint by clicking the “View selected footprint” button . Here is an example view of the footprint of a 0402 size surface mount component. 
