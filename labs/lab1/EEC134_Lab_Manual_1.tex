...
\reversemarginpar
\marginnote{\textbf{Pre-lab Assignment}\\\textbf{ 1.1} \\Due: \textcolor{red}{Sep.~25th, 2015}}  Please answer the following questions:
\begin{enumerate}[itemsep=0.1ex, label=\alph*)]
	...
	\item What do \textit{LSB} and \textit{MSB} mean in a DAC? For a 12-bit DAC with an output reference voltage $V_{ref}$ of 2\,V, how much voltage does an LSB correspond to? 
	

\end{enumerate}

\reversemarginpar
\marginnote{\textbf{Pre-lab Assignment}\\\textbf{ 1.2} \\Due: \textcolor{red}{Oct.~2th, 2015}}  Please answer the following questions:
\begin{enumerate}[itemsep=0.1ex, label=\alph*)]
...
\reversemarginpar
\marginnote{\textbf{Pre-lab Assignment}\\\textbf{ 1.3} \\Due: \textcolor{red}{Oct.~9th, 2015}}  Please answer the following questions:
\begin{enumerate}[itemsep=0.1ex, label=\alph*)]
	...
	\[
		x(t)=\sum_{k=-\infty}^\infty \left[ \left(\frac{2t}{T}-\frac{kT}{2}\right)\Pi(t-kT) + \left(1-\frac{2t}{T}\right)\Pi\left(t-\frac{(2k+1)T}{2}\right) \right],
	\]
	...
	If we pass the signal through an ideal low-pass filter that keeps only the first three harmonics (including the fundamental, that is, the fundamental, the 2nd harmonic, and the 3rd harmonic), what would the filter output signal look like in the time domain?
...
\item Use the batteries as your input power. Adjust $R_2$ until the output voltage is 5 V. For the LM317, you will need 8 AA batteries in total (2 packs). You should use the multimeter to measure the output voltage. 
\item Use the oscilloscope to measure the regulator output vs. time. Is the output stable? 
\item Now use the bench power supply as the input to the regulator, adjust $R_2$ until the output voltage is 5\,V. Observe the output voltage on the oscilloscope. Compare with the case when batteries were used as the input power. Which one gives a more stable output? What could have caused the difference?
...
\begin{enumerate}
	\item Using Fig.~\ref{fig:uno_sch} as a reference, connect the Arduino UNO to the MCP4921 DAC. Connect VDD to 5\,V and both AVSS and $\overline{\text{LDAC}}$ to ground. A \SI{0.1}{\micro\farad} and \SI{1}{\micro\farad} capacitors could be used at VREF to minimize noise at the reference. Connect the UNO with the computer using the USB cable, which is used both to program the UNO and power it up.  It is very important to keep your circuit (ICs, resistors, capacitors, wires, etc) organized. Use as short of a wire as possible between two connection points. Refer to Fig.~\ref{fig:uno_pic} for an example layout. 
