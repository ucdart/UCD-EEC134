...
\begin{figure}[h]
	\centering
	\includegraphics[width=5in]{lpf-sch}
...
\begin{enumerate}
	...
	\item Identify the gain stage in Fig.~\ref{fig:lpf_sch}. What is the expression of its gain? Notice the +2.5\,V bias does not affect this result, rather it simply shifts the bias of the overall amplifier since a single 5\,V supply (battery) is used vs a dual-rail (both positive and negative) supply.
...
\begin{enumerate}
	\item Identify the active LPF portion of the circuit in Fig.~\ref{fig:lpf_sch}. The overall filter consists of 2 low pass filters. What is the order of the overall filter and how is it determined from the schematic?
	
	\item Build the LPF circuit on your breadboard. 
	
	\item To measure just the filter response, disconnect R4 and R14 from the circuit (Fig.~\ref{fig:lpf_sch}). Input a 3\,Vpp signal in place of R4 and measure the output in place of R14.
	
	\item To measure just the filter response, disconnect R4 and R14 from the circuit (Fig.~\ref{fig:lpf_sch}). Input a 3\,Vpp signal in place of R4 and measure the output in place of R14. Tabulate and plot the frequency response from 100\,Hz to 50\,kHz. What is the cutoff frequency of the filter?
...
Design a PCB incorporating the circuits of Fig.~\ref{fig:lm317-sch}, \ref{fig:lt1009-sch}, \ref{fig:dac-sch}, and \ref{fig:lpf_sch}. In addition, we will add a resistive attenuator circuit just so that the output does not easily saturate. Fig.~\ref{fig:pcb-block} shows a high level block diagram of the PCB.

\begin{figure}[h]
	\centering
	\includegraphics[width=5in]{pcb-block}
...
\begin{enumerate}
	...
	\item For mounting ICs, we will use pin-headers that you can get from the TA. This shouldn't change the way you design the IC footprint. 
